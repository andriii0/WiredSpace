...
public class CommentServiceImpl implements CommentService {
    ...
    public CommentDTO createComment(Comment comment) {
        if (!postRepository.existsById(comment.getPostId())) {
            throw new PostNotFoundException("Post not found with id: " + comment.getPostId());
        }

        validateCommentContent(comment.getContent());

        User user = userRepository.getUserById(comment.getAuthorId())
                .orElseThrow(() -> new UserNotFoundException("User not found with id: " + comment.getAuthorId()));
    ...
    private void validateCommentContent(String content) {
        if (content == null || content.trim().isEmpty()) {
            throw new InvalidCommentContentException("Comment content must not be empty");
        }
